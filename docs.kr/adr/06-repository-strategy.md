...
다중 서비스 플랫폼은 코드 구성 전략에 대한 근본적인 결정이 필요하다. 이 결정은 다음에 영향을 미친다:
...
특정 배포 플랫폼은 독립 리포지토리를 요구하거나 강하게 선호한다:
...
이 제약은 생태계 확장(IDE 확장, CI 연동, CLI 도구) 계획 시 특히 중요하다.
...
각 서비스가 독립적인 라이프사이클을 가진 자체 리포지토리를 유지한다.
...
모든 서비스를 공유 도구와 함께 단일 리포지토리에 구성한다.
...
핵심 서비스는 Monorepo에, 특정 컴포넌트(라이브러리, 인프라)는 별도 리포에 구성한다.
