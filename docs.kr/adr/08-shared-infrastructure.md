...
> [English Version](../../docs/adr/08-shared-infrastructure.md)
...
여러 서비스로 구성된 분산 시스템에서, 데이터베이스와 캐시 같은 공유 인프라를 어떻게 관리할 것인가는 핵심적인 아키텍처 결정이다. 이 결정은 다음에 영향을 미친다:
...
모든 서비스가 단일 데이터베이스 인스턴스와 공유 캐시 계층에 연결.
...
각 서비스가 전용 데이터베이스 인스턴스를 소유하고 관리.
...
단일 데이터베이스 인스턴스에서 각 서비스별로 격리된 스키마 사용.
...
공유 주 데이터베이스와 서비스별 읽기 복제본 또는 캐시.
...
공유 캐싱(Redis/Memcached)도 유사한 트레이드오프:
